...
function Icon({ name, size = 20 }) {
  ...
  const paths = {
    ...
    trend: <><path d="M6 18 18 6"/><path d="M10 6h8v8"/></>,
    terrain: <><path d="M3 17c3.2-5.2 6.3-5.2 9.4 0 2.8 4.6 5.6 4.5 8.6-.2"/><path d="M4 10.5c2.7-3.5 5.5-3.5 8.3 0 2.7 3.4 5.2 3.5 7.7.4"/></>,
    blueprint: <><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M9 4v16M15 4v16M4 10h16M4 15h16"/></>,
    home: <><path d="m4 11 8-7 8 7"/><path d="M6 10v10h12V10M10 20v-6h4v6"/></>,
    cycle: <><path d="M19 8a8 8 0 0 0-13.5-2L3 8"/><path d="M3 4v4h4M5 16a8 8 0 0 0 13.5 2l2.5-2"/><path d="M21 20v-4h-4"/></>,
    briefcase: <><rect x="3" y="7" width="18" height="12" rx="2"/><path d="M9 7V5h6v2M3 12h18M10 12v2h4v-2"/></>,
    team: <><circle cx="9" cy="9" r="3"/><path d="M3.5 20a5.5 5.5 0 0 1 11 0M16 6.5a3 3 0 0 1 0 5.5M17 15a5 5 0 0 1 3.5 5"/></>,
    flag: <><path d="M6 21V4"/><path d="M6 5h11l-2 4 2 4H6"/></>,
    plus: <><path d="M12 5v14M5 12h14"/></>,
    minus: <path d="M5 12h14"/>,
  };
  return <svg className="ui-icon" width={size} height={size} viewBox="0 0 24 24" aria-hidden="true" focusable="false" {...common}>{paths[name]}</svg>;
}

const steps = [
  ['01', 'terrain', 'Аналізуємо', 'Оцінюємо ділянку, її потенціал та ринкові можливості.'],
  ['02', 'blueprint', 'Проєктуємо', 'Створюємо концепцію, що поєднує функціональність і природне середовище.'],
  ['03', 'home', 'Будуємо', 'Реалізуємо якісно, у строк і з контролем кожного етапу.'],
  ['04', 'trend', 'Запускаємо', 'Підключаємо до управління, бронювання та просування.'],
];
...
function App() {
  ...
  return <>
    <header className="site-header" data-header><div className="shell header-inner"><Logo /><nav className="desktop-nav"><a href="#concept">Концепція</a><a href="#landowners">Для власників землі</a><a href="#investors">Для інвесторів</a><a href="#faq">Питання</a></nav><button className="button button-outline header-button" onClick={() => request('consultation')}>Обговорити проєкт <span className="button-icon"><Icon name="trend" size={17}/></span></button><button className="menu-toggle" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}><span/><span/><span/><i>Меню</i></button></div><nav className={`mobile-nav ${menuOpen ? 'open' : ''}`}><a href="#concept" onClick={() => setMenuOpen(false)}>Концепція</a><a href="#landowners" onClick={() => setMenuOpen(false)}>Для власників землі</a><a href="#investors" onClick={() => setMenuOpen(false)}>Для інвесторів</a><a href="#faq" onClick={() => setMenuOpen(false)}>Питання та відповіді</a><button className="button button-primary" onClick={() => request('consultation')}>Обговорити проєкт</button></nav></header>
    <main id="top">
      <section className="hero section-dots"><div className="shell hero-grid"><div className="hero-copy reveal"><p className="overline"><i/> Девелопмент відпочинку</p><h1>Ваша земля<br/>може стати<br/><span>місцем сили.</span></h1><p className="hero-lead">Створюємо сучасні заміські проєкти — від потенціалу ділянки до працюючого бізнесу.</p><div className="hero-actions"><button className="button button-primary" onClick={() => request('land')}>Розкрити потенціал землі <span className="button-icon"><Icon name="arrow" size={17}/></span></button><button className="button button-ghost" onClick={() => request('investment')}>Стати інвестором <span className="button-icon"><Icon name="arrow" size={17}/></span></button></div></div><div className="hero-visual reveal"><div className="hero-arch"><div className="hero-cabin"/></div><img className="hero-lion" src={`${A}/green-lev-mascot-cutout.png`} alt="Зелений лев Green.Lev.Travel"/><div className="feature-chip chip-one"><i><Icon name="terrain" size={22}/></i><span>Аналіз ділянки</span></div><div className="feature-chip chip-two"><i><Icon name="blueprint" size={22}/></i><span>Власна концепція</span></div><div className="feature-chip chip-three"><i><Icon name="trend" size={22}/></i><span>Запуск і управління</span></div></div><div className="audience-panel reveal"><p className="panel-title">Партнерство, що створює цінність</p><button className="audience-card" id="landowners" onClick={() => request('land')}><span className="audience-icon"><Icon name="terrain" size={26}/></span><span><strong>Власникам землі</strong><small>Допоможемо розкрити потенціал ділянки та побудуємо прибутковий проєкт під ключ.</small></span><i><Icon name="arrow" size={20}/></i></button><span className="panel-or">або</span><button className="audience-card" id="investors" onClick={() => request('investment')}><span className="audience-icon"><Icon name="trend" size={26}/></span><span><strong>Інвесторам</strong><small>Добираємо перспективні локації та створюємо дохідні проєкти з прозорою моделлю співпраці.</small></span><i><Icon name="arrow" size={20}/></i></button></div></div></section>
      <section className="process section-contours" id="concept"><div className="shell"><div className="process-heading reveal"><div><p className="section-label">01 / Як це працює</p><h2>Від землі —<br/>до місця, куди <span>повертаються.</span></h2></div><img src={`${A}/green-lev-mascot-cutout.png`} alt="Маскот Green.Lev.Travel"/></div><div className="steps">{steps.map(([n,icon,title,text]) => <article className="step reveal" key={n}><div className="step-top"><b>{n}</b><i><Icon name={icon} size={21}/></i></div><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
      <section className="product-section"><div className="shell"><div className="product-card reveal"><div className="product-image"/><div className="product-copy"><p className="section-label light">02 / Наш продукт</p><h2>Архітектура,<br/>що працює разом<br/><span>із природою.</span></h2><p>Готова концепція заміських будинків для відпочинку, яка поєднує естетику, комфорт і прибутковість.</p><div className="metrics"><span><i><Icon name="home" size={18}/></i>35–60 м²</span><span><i><Icon name="cycle" size={18}/></i>4 сезони</span><span><i><Icon name="briefcase" size={18}/></i>готовий бізнес</span></div><div className="product-actions"><button className="button button-lime" onClick={() => request('presentation')}>Отримати презентацію <span className="button-icon"><Icon name="arrow" size={17}/></span></button><button className="button button-dark-outline" onClick={() => request('consultation')}>Отримати консультацію <span className="button-icon"><Icon name="arrow" size={17}/></span></button></div></div></div></div></section>
      <section className="partnership section-contours"><div className="shell"><p className="section-label">03 / Партнерство</p><h2 className="partnership-title reveal">Одна сильна ідея.<br/><span>Два способи стати її частиною.</span></h2><div className="partner-grid"><article className="partner-card land-card reveal"><div className="partner-card-top"><span className="round-icon"><Icon name="terrain" size={26}/></span><div><h3>Власникам землі</h3><p>Розкриємо потенціал ділянки та створимо проєкт під ключ.</p><button className="button button-primary small" onClick={() => request('land')}>Запропонувати землю <span className="button-icon"><Icon name="arrow" size={17}/></span></button></div></div><div className="landscape-lines"/></article><span className="plus"><Icon name="plus" size={24}/></span><article className="partner-card invest-card reveal"><div className="partner-card-top"><span className="round-icon"><Icon name="trend" size={26}/></span><div><h3>Інвесторам</h3><p>Знайдемо перспективну локацію та створимо дохідний актив.</p><button className="button button-primary small" onClick={() => request('investment')}>Стати партнером <span className="button-icon"><Icon name="arrow" size={17}/></span></button></div></div><div className="growth-chart"><svg viewBox="0 0 500 110" preserveAspectRatio="none"><path d="M0 104 C80 94,110 75,175 77 S280 54,340 42 S425 22,500 3"/></svg></div></article></div><div className="proof-strip reveal"><span><i><Icon name="team" size={19}/></i><strong>1 команда</strong></span><span><i><Icon name="blueprint" size={19}/></i><strong>4 етапи під ключ</strong></span><span><i><Icon name="flag" size={19}/></i><strong>1 спільний результат</strong></span></div></div></section>
      <section className="faq-section section-contours" id="faq"><div className="shell faq-grid"><div className="faq-intro reveal"><p className="section-label">04 / Питання та відповіді</p><h2>Відповідаємо<br/>до того, <span>як ви запитаєте.</span></h2><p>Зібрали найчастіші запитання власників та інвесторів, щоб ви швидко отримали чіткі відповіді.</p><div className="faq-actions"><button className="button button-ghost" onClick={() => request('presentation')}>Отримати презентацію <span className="button-icon"><Icon name="arrow" size={17}/></span></button><button className="button button-primary" onClick={() => request('consultation')}>Отримати консультацію <span className="button-icon"><Icon name="arrow" size={17}/></span></button></div></div><div className="accordion">{faqs.map(([q,a],i) => <article className={`faq-item ${openFaq === i ? 'active' : ''}`} key={q}><button type="button" aria-expanded={openFaq === i} onClick={() => setOpenFaq(openFaq === i ? -1 : i)}><span>{q}</span><i><Icon name={openFaq === i ? 'minus' : 'plus'} size={16}/></i></button><div className="faq-answer"><p>{a}</p></div></article>)}</div></div></section>
